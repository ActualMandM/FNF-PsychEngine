...
class NotesRGBSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		hsbText = new Alphabet(posX + 720, 0, "Red        Green      Blue", false);
		hsbText.scaleX = 0.6;
		hsbText.scaleY = 0.6;
		add(hsbText);
	...
	function changeSelection(change:Int = 0)
	{
		...
		if (curSelected >= ClientPrefs.arrowRGB.length)
			curSelected = 0;
		NotesHSVSubState.curSelected = curSelected;
	...
	function changeType(change:Int = 0)
	{
		...
		if (typeSelected > 2)
			typeSelected = 0;
		NotesHSVSubState.typeSelected = typeSelected;
